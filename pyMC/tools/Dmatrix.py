...
def get_euler_angles(c1, c2):
    ...
    if c1.ndim == 2 and c2.ndim == 2:
        ...
        if numpy.cross(c1[1], yp).dot(c1[2]) < 0:
            alpha = -alpha
        if yp.dot(c2[1])>1.0 and yp.dot(c2[1])< 1.0000000001:
...
def Dmatrix_r(j, alpha, beta, gamma):
    '''Wigner rotation D-matrix

    D_{mm'} = <jm|R(alpha,beta,gamma)|jm'>
    alpha, beta, gamma are Euler angles (in z-y-z convention)
    ...
    '''
    d = dmatrix_j(j, beta)
    ms = numpy.arange(-j, j+1.0, 1.0)
    D = numpy.einsum('i,ij,j->ij', numpy.exp(-1j*alpha*ms), d,
                     numpy.exp(-1j*gamma*ms))
    
    return D

def dmatrix_j(j, beta):
    '''Wigner small-d matrix (in z-y-z convention)'''
    c = numpy.cos(beta/2)
    s = numpy.sin(beta/2)
    facs = factorial(numpy.arange(2*j+1))
    cs = c**numpy.arange(2*j+1)
    ss = s**numpy.arange(2*j+1)

    mat = numpy.zeros((int(2*j+1),int(2*j+1)))
    for i_index,m1 in enumerate(numpy.arange(-j, j+1, 1.0)): #  m'
        for j_index,m2 in enumerate(numpy.arange(-j, j+1, 1.0)): #  m
            #:fac = sqrt( factorial(l+m1)*factorial(l-m1) \
            #:           *factorial(l+m2)*factorial(l-m2))
            #:for k in range(max(m2-m1,0), min(l+m2, l-m1)+1):
            #:    mat[i,j] += (-1)**(m1+m2+k) \
            ...
            #:               * factorial(m1-m2+k) * factorial(l-m1-k))
            #:mat[i,j] *= fac
            k = numpy.arange(int(max(m2-m1,0)), int(min(j+m2, j-m1))+1)
            tmp = (cs[int(2*j+m2-m1)-2*k] * ss[int(m1-m2)+2*k] /
                   (facs[int(j+m2)-k] * facs[k] * facs[int(m1-m2)+k] * facs[int(j-m1)-k]))

            mask = (int(m1-m2)+k & 0b1).astype(bool)
            # print(m1,m2,k,mask,int(m1+m2)+k)
            mat[i_index,j_index] -= tmp[ mask].sum()
            mat[i_index,j_index] += tmp[~mask].sum()

    ms = numpy.arange(-j, j+1, 1.0)
    msfac = numpy.sqrt(facs[(j+ms).astype(int)] * facs[(j-ms).astype(int)])
    mat *= numpy.einsum('i,j->ij', msfac, msfac)
    return mat
