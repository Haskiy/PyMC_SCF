...
'''
Author: Li Hao
Date: 2021-12-24 17:31:38
LastEditTime: 2022-03-21 08:33:40
LastEditors: Pu Zhichen
Description: This file just uses Gauss-Ledengre Grids

FilePath: \pyMC\pbc\gen_grid.py
Motto: A + B = C!
'''
...
from pyscf.dft.gen_grid import (sg1_prune, nwchem_prune, treutler_prune,
                                ...
                                BLKSIZE)
from pyMC.grids_util import grids_hss                     
from pyMC.grids_util.grids_hss import gen_atomic_grids_gauss_legendre
...
def get_becke_grids(cell, atom_grid={}, radi_method=dft.radi.gauss_chebyshev,
                    level=3, prune=nwchem_prune): # , legendre = None
...
# be consistent with the treatment in pbc_eval_gto (see issue 164).
    ...
    # if legendre is None:    # add by lihao
    atom_grids_tab = gen_atomic_grids(cell, atom_grid, radi_method, level, prune)
    # else:
    #     atom_grids_tab = legendre(cell, atom_grid, radi_method, level)  # add by lihao
    coords_all = []
    weights_all = []
...
class BeckeGrids(dft.gen_grid.Grids):
    ...
    def __init__(self, cell):
        self.cell = cell
        # self.legendre = None    # add by lihao
    ...
    def build(self, cell=None, with_non0tab=False):
        ...
        self.coords, self.weights = get_becke_grids(self.cell, self.atom_grid,
                                                    ...
                                                    level=self.level,
                                                    prune=self.prune,
                                                    legendre = self.legendre)    # add by lihao
        if with_non0tab:
...
class GLegendreGrids(grids_hss.Grids_hss):
    def __init__(self,cell):
        self.cell = cell
        dft.gen_grid.Grids.__init__(self, cell)

    def gen_atomic_grids_gauss_legendre(self, cell, atom_grid=None, radi_method=None,
        ...
        return gen_atomic_grids_gauss_legendre(cell, atom_grid, self.radi_method, level, prune, **kwargs)
