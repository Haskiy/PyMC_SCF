...
def AGEC_LDA_parallel_kernel(Mx, My, Mz, NX, rhop, xc_code, ni, index
    ...
    for idrct in range(init,finish):
        ...
        exc, exc_cor, vxcn, vxcs = ni.eval_xc_new_ASDP(xc_code, (rho_ahss, rho_bhss), spin=1,
                            ...
                            verbose=verbose)
        # # add by lihao                    
        # vxcn = numpy.empty((len(kpts),nso,nso),dtype=numpy.complex128)
        # vxcs = numpy.empty((len(kpts),nso,nso),dtype=numpy.complex128)
        # for i in range(0,len(kpts)):
        #     vxcn[i] = numpy.asarray(scipy.linalg.block_diag(vxcntmp[0,i],vxcntmp[1,i]), dtype=numpy.complex128)
        # vxcn[i][:nao,nao:] = vxcntmp[1,i].copy()
        # vxcn[i][nao:,:nao] = vxcntmp[2,i].copy()
        # for i in range(0,len(kpts)):
        #     vxcs[i] = numpy.asarray(scipy.linalg.block_diag(vxcstmp[0,i],vxcstmp[1,i]), dtype=numpy.complex128)
        # vxcs[i][:nao,nao:] = vxcstmp[1,i].copy()
        # vxcs[i][nao:,:nao] = vxcstmp[2,i].copy()
        # # add by lihao
        den = rho_ahss * weight
        # import pdb
        # pdb.set_trace()
...
class KNumInt(numint.NumInt):
    ...
    def eval_rho_ibp(self, cell, ao_kpts, dm_kpts, non0tab=None, xctype='LDA', hermi=0, verbose=None):
        nkpts = len(ao_kpts)
        rhoR = 0
        for k in range(nkpts):
    ...
    def _gen_rho_evaluator_ibp_pbc(self, cell, dms_kpts, hermi=0):
        if getattr(dms_kpts, 'mo_coeff', None) is not None:
            ...
            if isinstance(dms_kpts, numpy.ndarray) and dms_kpts.ndim == 3:
                for i in dms_kpts.shape[0]:
                    dms_kpts[i] = [dms_kpts[i]]
                if not hermi:
                    # For eval_rho when xctype==GGA, which requires hermitian DMs
                    dms_kpts[i] = [(dm+dm.conj().T)*.5 for dm in dms_kpts[i]]
            nao = dms_kpts[0,:].shape[0]
            ndms = len(dms_kpts)
            def make_rho(idm, ao, non0tab, xctype):
...
# NOTE to index grids.non0tab, the blksize needs to be the integer multiplier of BLKSIZE
    ...
    def nrk_new_ASDP_parallel(self, cell, grids, xc_code, dms, hermi=0, kpts=None, kpts_band=None,
        ...
        return nrk_new_ASDP_parallel(self, cell, grids, xc_code, dms, 1, 0,
                      hermi, kpts, kpts_band, max_memory, NX, verbose)
