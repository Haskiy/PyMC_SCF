...
from pyMC.gksm_util import numint_gksm as numint_mol
...
def eval_mat_hss(cell, ao, weight, vxc,
    ...
    if xctype == 'LDA':
        ...
        for ao_k1, ao_k2, mask, weight, coords \
            ...
            for idm in range(nset):
                ...
                factor = priciple_direction.WX[NX.shape[0]]
        ...
        for ao_k1, ao_k2, mask, weight, coords \
            ...
            for idm in range(nset):
                ...
                # ! Note that for Vmat, factor can be product here!
                # !     different from energy.
                wvaahss_factor = wvaahss_factor
                wvbbhss_factor = wvbbhss_factor
                wvabhss_r_factor = wvabhss_r_factor
                wvabhss_i_factor = wvabhss_i_factor
...
def nrk_new_ASDP_parallel(ni, cell, grids, xc_code, dms, spin=1, relativity=0, hermi=0,
    ...
    excsum = numpy.zeros(nset)
    # vmat will save in the order(alpha_alpha,alpha_beta,beta_alpha,beta_beta).
    vmat = numpy.zeros((4,nset,nkpts,nao,nao), dtype=numpy.complex128)
    # vmat = [0]*nset 
    # if kpts_band is not None:
    #     vmat = numpy.zeros((4,nset,n_band,nao,nao), dtype=numpy.complex128)
    ...
    if xctype == 'LDA':
        ...
        for ao_k1, ao_k2, mask, weight, coords \
            ...
            for idm in range(nset):
                ...
                # ~ parallel run spherical average
                for para in NX_list:
                    para_results.append(pool.apply_async(numint_mol.AGEC_LDA_parallel_kernel_num_stable,
                                                         (Mx, My, Mz, NX, rhop, xc_code, ni, para
                                                        , relativity, verbose, weight, ngrid, factor, THRESHOLD)))
                # ~ finisht the parallel part.
                pool.close()
                pool.join()
                # ~ get the final result
                ...
                idx_instable = s_norm_half>=THRESHOLD_lc*rhop
                N_instable = idx_instable.sum()
                
                idx_u_polar = rhop[idx_instable] - s_norm_half[idx_instable] <= THRESHOLD
                idx_d_polar = rhop[idx_instable] + s_norm_half[idx_instable] <= THRESHOLD
                print(idx_instable.sum(),idx_u_polar.sum(),idx_d_polar.sum())
...
def nrk_new_ASDP_parallel_ibp(ni, cell, grids, xc_code, dms, spin=1, relativity=0, hermi=0,
           kpts=None, kpts_band=None, max_memory=2000, NX=numpy.array([[0.0, 0.0, 1.0]]), THRESHOLD=1.0E-10, 
           THRESHOLD_lc = 0.99, verbose=None):
    '''Calculate UKS XC functional with Multi-directions framework
    and potential matrix on given meshgrids for a set of density matrices.
    ...
    if xctype == 'LDA':
        ...
        for ao_k1, ao_k2, mask, weight, coords \
            ...
            for idm in range(nset):
                ...
                factor = priciple_direction.WX[NX.shape[0]]
                for para in NX_list:
                    para_results.append(pool.apply_async(numint_mol.AGEC_GGA_intbypart_parallel_kernel_num_stable,
                                                         (Mx, My, Mz, NX, rhop, xc_code, ni, para
                                                        , relativity, verbose, weight, ngrid, factor, 
                                                        THRESHOLD)))
                # ~ finisht the parallel part.
                ...
                idx_instable = s_norm*0.5>=THRESHOLD_lc*rhop[0]
                N_instable = idx_instable.sum()
                NX_instable = numpy.zeros((N_instable,3))
                ...
                idx_u_polar = rho_bhss[0] <= THRESHOLD
                idx_d_polar = rho_ahss[0] <= THRESHOLD
                ...
                wvp = 0.5*(wva[0]+wvb[0])
                Bxc = numint_mol.cal_Bxc((rhop[:,idx_instable]*2, Mx[:,idx_instable], My[:,idx_instable], Mz[:,idx_instable]), 
                              NX_instable.transpose(1,0),
                    (wva, wvb, wvrho_nrho, wvnrho_nrho))
                Wxc = numint_mol.cal_Wxc((rhop[:,idx_instable]*2, Mx[:,idx_instable], My[:,idx_instable], Mz[:,idx_instable]), 
                              NX_instable.transpose(1,0),
                    (wva, wvb, wvrho_nrho, wvnrho_nrho))
                ...
                # pdb.set_trace()
                # nkpts = len(ao_k1)
                # ao_tmp = numpy.zeros((nkpts, ngrids))
                # for ikpt in range(nkpts):
                # ao_tmp[ikpt] = ao_k1[ikpt][0]
                # ngrids, nao = ao_k1[0].shape[-2]
                # ao_k1_tmp = numpy.zero(len(ao_k1),ngrids,nao)
                ao_k1 = numpy.array(ao_k1)
                ao_k1 = ao_k1.transpose(1,0,2,3)
                vmat[0,idm] += ni.eval_mat_hss(cell, ao_k1[0], weight, vaa_factor,
                                        mask, 'LDA', 1, verbose)
...
class NumInt(numint.NumInt):
    ...
    def _gen_rho_evaluator_ibp_pbc(self, cell, dms, hermi=0):
        if getattr(dms, 'mo_coeff', None) is not None:
            ...
            ndms = len(dms)
            def make_rho(idm, ao, non0tab, xctype):
                return self.eval_rho_ibp(cell, ao, dms[idm], non0tab, xctype, hermi=1)
...
# NOTE to index grids.non0tab, the blksize needs to be the integer multiplier of BLKSIZE
    ...
    # eval_xc_new_ASDP = numint_mol.eval_xc_new_ASDP
    ...
    #                               omega, verbose)[:4]
    #     # vrho, vsigma, vlapl, vtau = vxc
    #     # v2rho2, v2rhosigma, v2sigma2, v2lapl2, vtau2 = fxc
...
class KNumInt(numint.NumInt):
    ...
    def eval_rho_ibp(self, cell, ao_kpts, dm_kpts, non0tab=None, xctype='LDA', 
                     hermi=0, verbose=None):
        # import pdb
        # pdb.set_trace()
        nkpts = len(ao_kpts)
        rhoR = 0
        for k in range(nkpts):
            rhoR += eval_rho_ibp(cell, ao_kpts[k], dm_kpts[k], non0tab, xctype,
                             hermi, verbose)
        rhoR *= 1./nkpts
        return rhoR
    
    def _gen_rho_evaluator_ibp_pbc(self, cell, dms_kpts, hermi=0):
        # import pdb
        # pdb.set_trace()
        if getattr(dms_kpts, 'mo_coeff', None) is not None:
            #TODO: test whether dm.mo_coeff matching dm
            raise NotImplementedError("Not implemented yet")
            mo_coeff = dms_kpts.mo_coeff
            mo_occ = dms_kpts.mo_occ
            if isinstance(dms_kpts, numpy.ndarray) and dms.ndim == 3:
                mo_coeff = [mo_coeff]
                mo_occ = [mo_occ]
            nao = mo_coeff[0,:].shape[0]
            ndms = len(mo_occ)
            ...
            if isinstance(dms_kpts[0], numpy.ndarray) and dms_kpts[0].ndim == 2:
                dms_kpts = [dms_kpts]
            if not hermi:
                for i in range(dms_kpts[0].shape[0]):
                # For eval_rho when xctype==GGA, which requires hermitian DMs
                    dms_kpts[0][i] = (dms_kpts[0][i]+dms_kpts[0][i].conj().T)*.5 
                # dms_kpts[0][i] = [(dm[0][i]+dm[0][i].conj().T)*.5 for dm in dms_kpts[0]]
            nao = dms_kpts[0].shape[-1]
            ndms = len(dms_kpts)
            def make_rho(idm, ao, non0tab, xctype):
                return self.eval_rho_ibp(cell, ao, dms_kpts[idm], non0tab, xctype, hermi=1)
        return make_rho, ndms, nao
    ...
    def eval_mat_hss(self, cell, ao_kpts, weight, vxc,
                 non0tab=None, xctype='LDA', spin=0, verbose=None):
        nkpts = len(ao_kpts)
        nao = ao_kpts[0].shape[-1]
        dtype = numpy.result_type(*ao_kpts)
        mat = numpy.empty((nkpts,nao,nao), dtype=dtype)
        for k in range(nkpts):
            mat[k] = eval_mat_hss(cell, ao_kpts[k], weight, vxc,
                              non0tab, xctype, spin, verbose)
        return mat
...
# NOTE to index grids.non0tab, the blksize needs to be the integer multiplier of BLKSIZE
        ...
        for ip0 in range(0, ngrids, blksize):
            ...
            # self.eval_ao(cell, coords, kpts, deriv=deriv, non0tab=non0)
            if kpts_band is not None:
                ao_k1 = self.eval_ao(cell, coords, kpts_band, deriv=deriv, non0tab=non0)
            yield ao_k1, ao_k2, non0, weight, coords
            # yield ao_k2, non0, weight, coords
            ao_k1 = ao_k2 = None
    ...
    get_rho = get_rho
    # eval_xc_new_ASDP = numint_mol.eval_xc_new_ASDP
    # nrk_new_ASDP_parallel = nrk_new_ASDP_parallel
    # nrk_new_ASDP_parallel_ibp = nrk_new_ASDP_parallel_ibp

    def nrk_new_ASDP_parallel(self, cell, grids, xc_code, dms, hermi=0, kpts=None, kpts_band=None,
               max_memory=2000, NX=numpy.array([[0.0, 0.0, 1.0]]), verbose=None, **kwargs):
        if kpts is None:
            if 'kpt' in kwargs:
                sys.stderr.write('WARN: KNumInt.nr_uks function finds keyword '
                                 'argument "kpt" and converts it to "kpts"\n')
        ...
        return nrk_new_ASDP_parallel(self, cell, grids, xc_code, dms, 1, 0,
                      hermi, kpts, kpts_band, max_memory, NX, THRESHOLD  = kwargs['THRESHOLD']
                      , THRESHOLD_lc  = kwargs['THRESHOLD_lc'], verbose = verbose)
    
    def nrk_new_ASDP_parallel_ibp(self, cell, grids, xc_code, dms, hermi=0, kpts=None, kpts_band=None,
               max_memory=2000, NX=numpy.array([[0.0, 0.0, 1.0]]), verbose=None, **kwargs):
        if kpts is None:
            if 'kpt' in kwargs:
                sys.stderr.write('WARN: KNumInt.nr_uks function finds keyword '
        ...
        return nrk_new_ASDP_parallel_ibp(self, cell, grids, xc_code, dms, 1, 0,
                      hermi, kpts, kpts_band, max_memory, NX, kwargs['THRESHOLD']
                      , kwargs['THRESHOLD_lc'], verbose)
        
    def eval_xc_new_ASDP(self, xc_code, rho, spin=1, relativity=0, deriv=1, omega=None,
                verbose=None, ibp = False, THRESHOLD = 1.0E-10):
        """calculate the xc functional used in new ASDP formalism.
        ...
        xctype = self._xc_type(xc_code)
        if xctype == 'LDA':
            idx_u_polar = rho[1] <= THRESHOLD
            idx_d_polar = rho[0] <= THRESHOLD

        elif xctype == 'GGA':
        
            idx_u_polar = rho[1][0] <= THRESHOLD
            idx_d_polar = rho[0][0] <= THRESHOLD
        ...
        if ibp:
            ...
            # Ns_Ns_Ns : (10, ngrid) xxx xxy xxz xyy xyz xzz yyy yyz yzz zzz
            n_s_Ns, s_s_Ns, n_s_Nn, s_s_Nn, n_Ns_Ns, s_Ns_Ns, s_Nn_Ns, \
                n_Nn_Ns, s_Nn_Nn, Nn_Ns_Ns, Ns_Ns_Ns, Nn_Nn_Ns = numint_mol.get_kxc_in_s_n_num_stable(rho, v2rhosigma, v2sigma2, 
                                                                                           kxc, idx_u_polar, idx_d_polar ) 
            ...
            if xctype == 'LDA':
                ...
                # construct temporary variables
                # ^ NUMERICASL STABLE
                wva[:3,idx_d_polar] = 0.0
                wvb[1:,idx_u_polar] = 0.0
                u_u[idx_d_polar] = 0.0
                u_d[idx_d_polar] = 0.0
                u_d[idx_u_polar] = 0.0
                d_d[idx_u_polar] = 0.0
